...
var debug = util.debuglog('httptunnel');
...
exports.createPersistentClient = function(tunnelServerHost, port, readyCallback) {
  ...
  if (t.send_queue.length >0)
      console.log("quene length:",t.send_queue.length)
  t.interval = setInterval(function() {
    ...
    var json = JSON.stringify(t.send_queue);
    req.write(json);
    req.end();
    // if (t.send_queue.length >0)
      // console.log(json)
  ...
  t.httpServer = http.createServer(function(req, res) {
    ...
    try {
      ...
      req.on('end', function(d) {
        ...
        if (d) buffer += d;
        // now is base64 yet 
        // example: [{"method":"data","stream_id":0,"data":"MQ0K"}] 
        // MQ0K is base64 string of "1"
        // chunk.data is actually DATA TCP procotols
        ...
        res.writeHead(200);
        // if (buffer.toString("utf-8")!="[]"){
          // debug("JSON back:",JSON.stringify(t.send_queue))
        // }
        //  t.send_queue is send quene to tcp client from tcp server
        // example :  [{"method":"data","stream_id":0,"data":"c2VydmVyIGVjaG86Mg0K"}]
        // c2VydmVyIGVjaG86Mg0K is base64 of server echo:2
